...
class DilatedGatedAttention(nn.Module):
    def __init__(self, in_c, out_c, k_size=3, dilated_ratio: list[int] = None):
        super().__init__()

        if dilated_ratio is None:
            dilated_ratio = [7, 5, 2, 1]
...
class MALUNet(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=3, c_list: list[int]=None,
