...
class TransFuse_S(nn.Module):
    def __init__(self,
        ...
        if pretrained_model_path is not None:
            self.resnet.load_state_dict(torch.load(os.path.join(pretrained_model_path, 'resnet34-333f7ec4.pth')))
...
class TransFuse_L(nn.Module):
    def __init__(self,
        ...
        if pretrained_model_path is not None:
            self.resnet.load_state_dict(torch.load(os.path.join(pretrained_model_path, 'resnet50-19c8e357.pth')))
        self.resnet.fc = nn.Identity()
        self.resnet.layer4 = nn.Identity()
