...
def deit_base_patch16_224(pretrained_model_path=None, image_size=(192, 256), **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained_model_path is not None:
        ckpt = torch.load(os.path.join(pretrained_model_path, 'deit_base_patch16_224-b5f2ef4d.pth'))
        model.load_state_dict(ckpt['model'], strict=False)
...
def deit_base_patch16_384(pretrained_model_path=None, image_size=(192, 256), **kwargs):
    ...
    model.default_cfg = _cfg()
    if pretrained_model_path is not None:
        ckpt = torch.load(os.path.join(pretrained_model_path, 'deit_base_patch16_384-8de9b5d1.pth'))
