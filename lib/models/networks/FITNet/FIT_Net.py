...
from .DST_block_IA import DST_block_IA
...
class FITNet(L.LightningModule):
    def __init__(self, pretrained=True, num_classes=2, **kwargs):
        super(FITNet, self).__init__()
        self.model = ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, **kwargs)
        if pretrained:
            pretrained_model = models_torch.resnet50(pretrained=pretrained)
            pretrained_dict = pretrained_model.state_dict()
